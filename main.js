import {convertDecimalToBinary, convertDecimalToBinaryPowers} from './common.js';

// pure
const numberBoxComponent = (number) => {
  // let div = document.createElement("div");
  return `<div style='min-width: 3rem; height: 3rem; display: flex; justify-content: center; align-items: center; border: 1px solid #008eff; border-radius: 10px; margin: 2px 2px;'>${number}</div>`;
};
...
(() => {
  ...
  const renderNumbersOf2 = (number, boxCount) => {
    ...
    indices = indices.map((elem) => `<span>2<sup>${elem}</sup></span>`); // 2^0  2^1  2^2  2^3 ....
